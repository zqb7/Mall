		...
		<scroll-view class="container" style="height: 100%;">
		  <view class="search-header">
		    <view class="input-box">
		      ...
		      <input name="input" class="keywrod" focus="true" :value="keyword" confirm-type="text" @input="inputChange" @focus="inputFocus" @confirm="onKeywordConfirm" :placeholder="defaultKeyword.keyword" />
		      <image class="del" v-if="keyword" @click="clearKeyword" src="http://nos.netease.com/mailpub/hxm/yanxuan-wap/p/20150730/style/img/icon-normal/clearIpt-f71b83e3c2.png"></image>
		    </view>
		  ...
		  <view class="no-search" v-if="!searchStatus">
		    ...
		    <view class="h">
		      ...
		      <image class="icon" @click="clearHistory" src="http://nos.netease.com/mailpub/hxm/yanxuan-wap/p/20150730/style/img/icon-normal/del1-93f0a4add4.png"></image>
		    </view>
		    <view class="b">
		      <view class="item" @click="onKeywordTap"v-for="(item,key) in historyKeyword" :key="key" hover-class="navigator-hover">{{item}}</view>
		    </view>
		  </view>
		  <view class="serach-keywords search-hot" v-if="!keyword">
		    ...
		    <view class="b">
		      <view class="item" :class="item.is_hot === 1 ? 'active' : ''" hover-class="navigator-hover" @click="onKeywordTap" v-for="(item,key) in hotKeyword" :key="key">{{item.keyword}}</view>
		    </view>
		  </view>
		  <view class="shelper-list" v-if="keyword">
		    <view class="item" hover-class="navigator-hover" v-for="(item, key) in helpKeyword" :key="key" @click="onKeywordTap">{{item}}</view>
		  </view>
		  </view>
		  ...
		  <view class="search-result" v-if="searchStatus && goodsList.length">
		    <view class="sort">
		    <view class="sort-box">
		      <view class="item" :class="currentSortType == 'default' ? 'active' : ''" @click="openSortFilter" id="defaultSort">
		        <text class="txt">综合</text>
		      </view>
		      <view class="item by-price" :class="currentSortType == 'price' ? 'active' : ''"  @click="openSortFilter" id="priceSort">
		        <text class="txt">价格</text>
		      </view>
		      <view class="item" :class="currentSortType == 'category' ? 'active' : ''" @click="openSortFilter" id="categoryFilter">
		        <text class="txt">分类</text>
		      </view>
		    </view>
		    <view class="sort-box-category" v-if="categoryFilter">
		      <view class="item" :class="item.checked ? 'active' : ''" v-for="(item, key) in filterCategory" :key="cate-item.id" @click="selectCategory">{{item.name}}</view>
		    </view>
		  </view>
		  <view class="cate-item">
		    <view class="b">
		      <navigator class="item" :class="(iindex + 1) % 2 == 0 ? 'item-b' : ''" url="/pages/goods/goods?id=iitem.id" v-for="(iitem, iindex) in goodsList" :key="iindex">
